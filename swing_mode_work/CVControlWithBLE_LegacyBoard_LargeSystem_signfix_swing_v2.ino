...
int AlignCounter = 0;
int OpMode = 0; //Nav Mode: tumbling, spiral 45, spiral 90, spiral 135
...
float VxDir[] = { 1, 1, 1, 1}; //Determines how Vx and Vy is introduced into the RRF calculator
float VyDir[] = { 1,-1,-1,-1};
...
void UART0_RX_CB()
{
  ...
  if (uart_buffer.length() > 0) {
    ...
    if (xSemaphoreTake(uart_buffer_Mutex, portMAX_DELAY)) {
      ...
      {        
        int U_VxD1 = uart_buffer[0];
        int U_VxD2 = uart_buffer[1];
        int U_VyD1 = uart_buffer[2];
        int U_VyD2 = uart_buffer[3]; 
        ...
        U_VxD1 = (U_VxD1 & 0xF)<<8;
        U_VyD1 = (U_VyD1 & 0xF)<<8;
        int U_VxD = U_VxD1+U_VxD2;
        int U_VyD = U_VyD1+U_VyD2;
        
        //FxxxFxxx: 3D manual attitute control
        //D1 & D2 sets polar angle, D3 & D4 sets azimuth angle
        if(U_Type1 == 15 && U_Type2 == 15)
        {
            AlignPolar = (U_VxD/4095)*180;
            AlignAzimuth = (U_VyD/4095)*360;
        ...
        // UART0.println(U_VxD1+U_VxD2);
        // UART0.println(U_VyD1+U_VyD2);
        // UART0.println(Vx);
        // UART0.println(Vy);
