  ...
  if(Vx==0 && Vy==0)//When output velocity is 0, device enters low output position maintain mode
  ...
  else //Auto control
    {
      ...
      else //Mode 4: Swing
      {
        ...
        // Swing the magnetic north pole left and right around the heading
        // direction in the XY plane instead of pitching along the heading.
        VRot(0) = cos(SwingRad)*Vx_N - sin(SwingRad)*Vy_N;
        VRot(1) = sin(SwingRad)*Vx_N + cos(SwingRad)*Vy_N;
        VRot(2) = 0;
      }
