...
app.PendingClose = false;
app.ConfigFile = fullfile(fileparts(mfilename('fullpath')), 'SensorDomainAcquisitionApp_lastSettings.mat');
...
motionRow = motionRow + 1;
app.ZStartHeightEditField = addNumericField(motionGrid, motionRow, 1, 'Z Start Height', 0);
motionRow = motionRow + 1;
...
updateStateBar();
    ...
    function onStart(~, ~)
        ...
        app.IsRunning = false;
        if app.PendingClose && isvalid(app.UIFigure)
            saveAppSettings();
            delete(app.UIFigure);
        end
    end
    ...
    function saveAppSettings()
        settings = struct();
        settings.xDistance = app.XDistanceEditField.Value;
        settings.xStep = app.XStepEditField.Value;
        settings.xDirection = app.XDirectionDropDown.Value;
        settings.xModel = app.XModelDropDown.Value;
        settings.yDistance = app.YDistanceEditField.Value;
        settings.yStep = app.YStepEditField.Value;
        settings.yDirection = app.YDirectionDropDown.Value;
        settings.yModel = app.YModelDropDown.Value;
        settings.zDistance = app.ZDistanceEditField.Value;
        settings.zStep = app.ZStepEditField.Value;
        settings.zDirection = app.ZDirectionDropDown.Value;
        settings.zModel = app.ZModelDropDown.Value;
        settings.zStartHeight = app.ZStartHeightEditField.Value;
        settings.motionPause = app.MotionPauseEditField.Value;
        settings.moveTol = app.MotionToleranceEditField.Value;
        settings.movePollPause = app.MotionPollPauseEditField.Value;
        settings.ip = app.InstrumentIPEditField.Value;
        settings.timeout = app.TimeoutEditField.Value;
        settings.startFreq = app.StartFreqEditField.Value;
        settings.stopFreq = app.StopFreqEditField.Value;
        settings.points = app.SweepPointsEditField.Value;
        settings.ifbw = app.IFBWEditField.Value;
        settings.power = app.PowerEditField.Value;
        settings.cmd = app.SweepCommandEditField.Value;
        settings.traceOrder = app.TraceOrderDropDown.Value;
        settings.bgIfbw = app.BackgroundIFBWEditField.Value;
        settings.bgPower = app.BackgroundPowerEditField.Value;
        settings.aiDev = app.AIDeviceEditField.Value;
        settings.aiChan = app.AIChannelEditField.Value;
        settings.aiMin = app.AIRangeMinEditField.Value;
        settings.aiMax = app.AIRangeMaxEditField.Value;
        settings.aiRate = app.AIRateEditField.Value;
        settings.aoDev = app.AODeviceEditField.Value;
        settings.aoChan = app.AOChannelEditField.Value;
        settings.aoStart = app.AOStartEditField.Value;
        settings.aoStop = app.AOStopEditField.Value;
        settings.aoStep = app.AOStepEditField.Value;
        settings.aoMin = app.AORangeMinEditField.Value;
        settings.aoMax = app.AORangeMaxEditField.Value;
        settings.aoTol = app.AOToleranceEditField.Value;
        settings.aoPause = app.AOPollPauseEditField.Value;
        settings.py = app.PythonExeEditField.Value;
        settings.sdk = app.SDKPathEditField.Value;
        settings.saveFolder = app.SaveFolderEditField.Value;
        settings.prefix = app.PrefixEditField.Value;
        settings.bgLift = app.BackgroundMoveHeightEditField.Value;
        try
            save(app.ConfigFile, 'settings');
        catch
        end
    end
    ...
    function loadAppSettings()
        ...
        try
            data = load(app.ConfigFile, 'settings');
            if ~isfield(data, 'settings')
                return;
            end
            settings = data.settings;
            setIfPresent(app.XDistanceEditField, settings, 'xDistance');
            setIfPresent(app.XStepEditField, settings, 'xStep');
            setIfPresent(app.XDirectionDropDown, settings, 'xDirection');
            setIfPresent(app.XModelDropDown, settings, 'xModel');
            setIfPresent(app.YDistanceEditField, settings, 'yDistance');
            setIfPresent(app.YStepEditField, settings, 'yStep');
            setIfPresent(app.YDirectionDropDown, settings, 'yDirection');
            setIfPresent(app.YModelDropDown, settings, 'yModel');
            setIfPresent(app.ZDistanceEditField, settings, 'zDistance');
            setIfPresent(app.ZStepEditField, settings, 'zStep');
            setIfPresent(app.ZDirectionDropDown, settings, 'zDirection');
            setIfPresent(app.ZModelDropDown, settings, 'zModel');
            setIfPresent(app.ZStartHeightEditField, settings, 'zStartHeight');
            setIfPresent(app.MotionPauseEditField, settings, 'motionPause');
            setIfPresent(app.MotionToleranceEditField, settings, 'moveTol');
            setIfPresent(app.MotionPollPauseEditField, settings, 'movePollPause');
            setIfPresent(app.InstrumentIPEditField, settings, 'ip');
            setIfPresent(app.TimeoutEditField, settings, 'timeout');
            setIfPresent(app.StartFreqEditField, settings, 'startFreq');
            setIfPresent(app.StopFreqEditField, settings, 'stopFreq');
            setIfPresent(app.SweepPointsEditField, settings, 'points');
            setIfPresent(app.IFBWEditField, settings, 'ifbw');
            setIfPresent(app.PowerEditField, settings, 'power');
            setIfPresent(app.SweepCommandEditField, settings, 'cmd');
            setIfPresent(app.TraceOrderDropDown, settings, 'traceOrder');
            setIfPresent(app.BackgroundIFBWEditField, settings, 'bgIfbw');
            setIfPresent(app.BackgroundPowerEditField, settings, 'bgPower');
            setIfPresent(app.AIDeviceEditField, settings, 'aiDev');
            setIfPresent(app.AIChannelEditField, settings, 'aiChan');
            setIfPresent(app.AIRangeMinEditField, settings, 'aiMin');
            setIfPresent(app.AIRangeMaxEditField, settings, 'aiMax');
            setIfPresent(app.AIRateEditField, settings, 'aiRate');
            setIfPresent(app.AODeviceEditField, settings, 'aoDev');
            setIfPresent(app.AOChannelEditField, settings, 'aoChan');
            setIfPresent(app.AOStartEditField, settings, 'aoStart');
            setIfPresent(app.AOStopEditField, settings, 'aoStop');
            setIfPresent(app.AOStepEditField, settings, 'aoStep');
            setIfPresent(app.AORangeMinEditField, settings, 'aoMin');
            setIfPresent(app.AORangeMaxEditField, settings, 'aoMax');
            setIfPresent(app.AOToleranceEditField, settings, 'aoTol');
            setIfPresent(app.AOPollPauseEditField, settings, 'aoPause');
            setIfPresent(app.PythonExeEditField, settings, 'py');
            setIfPresent(app.SDKPathEditField, settings, 'sdk');
            setIfPresent(app.SaveFolderEditField, settings, 'saveFolder');
            setIfPresent(app.PrefixEditField, settings, 'prefix');
            setIfPresent(app.BackgroundMoveHeightEditField, settings, 'bgLift');
            setStatus('Loaded last-used settings');
        catch
        end
    end

    function setIfPresent(control, settings, fieldName)
        if isfield(settings, fieldName)
            control.Value = settings.(fieldName);
        end
    end
    ...
    function params = readParams()
        ...
        params.zStartHeight = app.ZStartHeightEditField.Value;
    ...
    function acquireBackground(params)
        ...
        sData = collectSDataFromVisa(app.vi, params.cmd);
    ...
    function runAcquisition(params)
        ...
        for iz = 1:numel(zPos)
            ...
            for iy = 1:numel(yPos)
                ...
                for ix = 1:numel(xPos)
                    if app.ShouldStop, break; end
                    xTarget = app.StartPosition(1) + xPos(ix);
                    if ix == 1
                        moveAxisAbsoluteChecked(app.TMC1, app.TMCHandle, 1, xTarget, params.moveTol, params.movePollPause);
                    else
                        moveAxisAbsolute(1, xTarget, params.motionPause);
                    end
            ...
            moveAxisAbsoluteChecked(app.TMC1, app.TMCHandle, 1, app.StartPosition(1), params.moveTol, params.movePollPause);
            moveAxisAbsoluteChecked(app.TMC1, app.TMCHandle, 2, app.StartPosition(2), params.moveTol, params.movePollPause);
    ...
    function saveZFile(params, iz, zNow, xPos, yPos, allS21Data, allS11Data, scanLog)
        ...
        zDisplay = zNow + params.zStartHeight;
        s21Without = allS21Data - reshape(app.BackgroundS21, [], 1, 1);
        s11Without = allS11Data - reshape(app.BackgroundS11, [], 1, 1);
        metadata = struct('savedAt', char(datetime('now', 'Format', 'yyyy-MM-dd''T''HH:mm:ss')), ...
            'zIndex', iz - 1, 'zPositionMM', zDisplay, 'zStagePositionMM', zNow, 'xPositions', xPos, 'yPositions', yPos, ...
        ...
        fileName = sprintf('%s_z%02d_%0.3fmm.mat', params.prefix, iz - 1, zDisplay);
        fileName = strrep(fileName, '-', 'm');
        save(fullfile(params.saveFolder, fileName), 'freqData', 'allS21Data', 'allS11Data', 's21Without', 's11Without', 'scanLog', 'metadata');
    end
    ...
    function idn = connectMainVNA(params)
        if ~isempty(app.vi)
            try
                if strcmp(app.vi.Status, 'open')
                    set(app.vi, 'Timeout', params.timeout);
                    fprintf(app.vi, '*IDN?');
                    idn = strtrim(fscanf(app.vi));
                    if isempty(idn)
                        idn = params.ip;
                    end
                    app.VNAConnected = true;
    ...
    function ensureLockedVNAConnection(params)
        if isempty(app.vi)
            app.VNAConnected = false;
            updateStateBar();
            error('VNA connection is missing. Click Lock Params again.');
        end
        try
            if ~strcmp(app.vi.Status, 'open')
                app.VNAConnected = false;
                updateStateBar();
                error('VNA connection is closed. Click Lock Params again.');
            end
            set(app.vi, 'Timeout', params.timeout);
            fprintf(app.vi, '*IDN?');
            fscanf(app.vi);
            ...
            app.VNAConnected = false;
            updateStateBar();
            error('VNA connection check failed. Click Lock Params again.');
        end
    end

    function cleanupMotionAndDAQ()
    ...
    function cleanupForeignInstruments()
        obj = instrfind;
        if isempty(obj)
            return;
        end
        for idx = 1:numel(obj)
            currentObj = obj(idx);
            try
                if ~isempty(app.vi) && isequal(currentObj, app.vi)
                    continue;
                end
            catch
            end
            try
                if strcmp(currentObj.Status, 'open')
                    fclose(currentObj);
                end
            catch
            end
    ...
    function updatePosition(xyz)
        displayXYZ = xyz;
        displayXYZ(3) = displayXYZ(3) + app.ZStartHeightEditField.Value;
        app.PositionValueLabel.Text = sprintf('(%.2f, %.2f, %.2f) mm', displayXYZ(1), displayXYZ(2), displayXYZ(3));
    end
