classdef E5080A_NI_TMC_Microrobot < handle
    properties
        Settings
        SettingsFile
        Fig
    ...
    methods
        function app = E5080A_NI_TMC_Microrobot()
            app.SettingsFile = fullfile(fileparts(mfilename('fullpath')), 'E5080A_NI_TMC_Microrobot_settings.mat');
    ...
    methods (Access = private)
        ...
        function buildUI(app)
            app.Fig = figure('Name', 'E5080A NI TMC Microrobot', 'NumberTitle', 'off', 'MenuBar', 'none', 'ToolBar', 'none', 'Color', [0.97 0.97 0.98], 'Position', [120 70 1460 920], 'ResizeFcn', @(~,~) app.updateLayout(), 'CloseRequestFcn', @(~,~) app.onClose());
            ...
            app.ControlPanel = uipanel(app.Fig, 'Title', 'Parameters', 'Units', 'normalized', 'BackgroundColor', 'white');
        ...
        function updateLayout(app)
            ...
            controlMinW = 0.40;
            imageW = min(topH * figAspect, 1 - 2 * margin - gap - controlMinW);
            controlX = margin + imageW + gap;
            controlW = 1 - margin - controlX;

            set(app.ImagePanel, 'Position', [margin topY imageW topH]);
            set(app.ControlPanel, 'Position', [controlX topY controlW topH]);
            ...
            rightFrac = controlW / totalBottomW;
